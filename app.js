...
async function setupDummyAuth() {
    try {
        if (!localStorage.getItem('demoProviderId')) {
            ...
            if (pData.length > 0) {
                localStorage.setItem('demoProviderId', pData[0]._id);
            } else {
                // 👉 สร้าง provider อัตโนมัติ
                const newP = await fetch(`${API_BASE}/providers/create`, {
                    method: 'POST',
                ...
                const created = await newP.json();
                localStorage.setItem('demoProviderId', created._id);
            }
        }
